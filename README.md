...
The frontend is a modern React application built with Vite, Tailwind CSS v4, and Recharts, located in the `frontend/` directory.

### Features
- **Sleek Dashboard UI:** Glassmorphism design and micro-animations for a premium feel.
- **Client-Side Validation:** Form inputs are validated before hitting the backend (e.g., date ranges max 31 days).
- **Data Visualization:** Recharts is used to plot daily temperature trends (Max/Min), providing immediate insight into the raw JSON data.
- **Paginated Data Table:** A responsive data table with 10/20/50 row pagination options to view raw metrics.
...
> **Note:** The frontend expects the backend to be running on `http://localhost:8000`. If your backend is running on a different port, update the `baseURL` in `frontend/src/api/config.js`.

### Deployment — GitHub Pages (Automated)

The frontend is automatically deployed to a live URL using GitHub Pages via GitHub Actions.

1. Any push to the `master` branch triggers the `.github/workflows/deploy.yml` action.
2. The action builds the Vite project and deploys it to the `gh-pages` environment.
3. Your live dashboard is available at: `https://prime3436.github.io/inrisk-weather-backend/`

> **Note on Backend Connection:** When testing the live frontend, ensure your backend is either running locally (and you have disabled mixed content blocking) or deployed to Cloud Run. You can pass the live backend URL to the frontend build by setting the `VITE_API_URL` secret/env var in GitHub Actions or locally in a `.env` file.
